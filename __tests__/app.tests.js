...
test("GET articles should also return the correct comment count for each article", () => {
  return request(app)
    .get("/api/articles")
    .expect(200)
    .then((res) => {
      const output = res.body.articles[0];
      console.log(output);
      const actualCommentCount = Number(output.comment_count);
      const expectedCommentCount = 2;
      expect(actualCommentCount).toEqual(expectedCommentCount);
    });
});
